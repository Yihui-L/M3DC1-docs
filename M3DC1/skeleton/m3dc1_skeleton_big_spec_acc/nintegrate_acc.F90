...
implicit none
  ...
  logical :: surface_int
!$acc declare create (surface_int)
...
!$OMP THREADPRIVATE(weight_79,xi_79,zi_79,eta_79,norm79) 
...
!$acc declare copyin  (alpha_12, beta_12, gamma_12, area_weight_12)
  ...
  !$acc declare create (xi_79, zi_79, eta_79, x_79, z_79, phi_79)
  !$acc declare create (weight_79, norm79, npoints)
  !$acc declare copyin (line_weight_2, line_weight_3, line_weight_5)
  !$acc declare copyin (delta_2, delta_3, delta_5)
...
!======================================================================
subroutine extrude_quadrature(d, npol,  &
           ntor,weight_79_l,xi_79_l,zi_79_l,eta_79_l,norm79_l)
  use math
  use mesh
  implicit none

!$acc routine 
!$acc routine (safestop) bind (d_safestop)
  ...
  case(0)
     npoints = npol
     zi_79(1:npol) = 0.
     weight_79_l(1:npol) = weight_79_l(1:npol)*d
     return
  ...
  j = npol + 1
  do i=2, ntor
     xi_79_l(j:j+npol-1) = xi_79_l(1:npol)
     eta_79_l(j:j+npol-1) = eta_79_l(1:npol)
     weight_79_l(j:j+npol-1) = weight_79_l(1:npol)
     if(surface_int) then
        norm79_l(j:j+npol-1,:) = norm79_l(1:npol,:)
     endif
     j = j + npol
  end do
  ...
  j = 1
  do i=1, ntor
     zi_79_l(j:j+npol-1) = phi(i)
     weight_79_l(j:j+npol-1) = weight_79_l(j:j+npol-1)*wt(i)
     j = j + npol
  end do
...
subroutine define_boundary_quadrature(ielm, iedge, npol, ntor, normal, idim)
  ...
  surface_int = .true.
!$acc update device (surface_int)

#ifdef USE3D
  call extrude_quadrature(d%d,npol,ntor,weight_79,&
           xi_79,zi_79,eta_79,norm79)
...
!=====================================================
subroutine define_element_quadrature(ielm, pol_gauss, tor_gauss,weight_79_l, &
        xi_79_l, zi_79_l, eta_79_l, norm79_l)
  use mesh

!$acc routine seq
  integer, intent(in), value :: ielm, pol_gauss, tor_gauss
  real, dimension(MAX_PTS) :: xi_79_l, zi_79_l, eta_79_l, weight_79_l
  real, dimension(MAX_PTS,2) :: norm79_l
...
!  case(12)
     call area_to_local(12,                          &
          ...
          d%a, d%b, d%c, xi_79_l, eta_79_l, weight_79_l)
!  case(25)
...
!!$acc update device (surface_int)
#ifdef USE3D
  call extrude_quadrature(d%d, pol_gauss, tor_gauss,weight_79_l,&
           xi_79_l,zi_79_l,eta_79_l,norm79_l)
#else
  npoints = pol_gauss
#endif
...
!==============================================
real function int0(weight_79_l)

  implicit none
  real, dimension(MAX_PTS) :: weight_79_l
  integer :: k
  real :: ksum
!$acc routine seq
  ksum = 0.
  do k=1, npoints
     ksum = ksum + weight_79_l(k)
  enddo
...
!==============================================
real function int1(vari,weight_79_l)

  implicit none

  real, dimension(npoints), intent(in) :: vari
  real, dimension(MAX_PTS) :: weight_79_l
!$acc routine seq
  ...
  ksum = 0.
  do k=1, npoints
     ksum = ksum + vari(k)*weight_79_l(k)
  enddo
...
!==============================================
!DIR$ ATTRIBUTES FORCEINLINE :: int2
!real function int2(vari,varj)
function int2(vari,varj)
  implicit none
!$acc routine
    real, dimension(dofs_per_element) :: int2

! real, dimension(npoints), intent(in) :: vari, varj
  real, dimension(npoints,dofs_per_element), intent(in) :: vari
  real, dimension(npoints), intent(in) :: varj

  integer :: k,i
  real :: ks, ksum(dofs_per_element)
...
!  ksum(i) = 0.
  ks=0.
  do k=1, npoints
     ks = ks + vari(k,i)*varj(k)*weight_79(k)
!     ksum(i) = ksum(i) + vari(k,i)*varj(k)*weight_79(k)
  enddo
  ksum(i)=ks
enddo
...
!==============================================
!DIR$ ATTRIBUTES FORCEINLINE :: int3
real function int3(vari,varj,vark)
  implicit none

  !$acc routine
  real, dimension(npoints), intent(in) :: vari, varj, vark

  integer :: k
  real :: ksum

  ksum = 0.
  !!$acc parallel loop reduction(+:ksum)
  do k=1, npoints
...
!==============================================
real function int4(vari,varj,vark,varl)

  implicit none

  real, dimension(npoints), intent(in) :: vari, varj, vark, varl
...
!==============================================
real function int5(vari,varj,vark,varl,varm)

  implicit none

  real, dimension(npoints), intent(in) :: vari, varj, vark, varl, varm
