...
!==================================================
subroutine precalculate_terms(xi,zi,eta,co,sn)
  use element
  ...
  subroutine define_basis(itri)
    ...
    call dgemm('N','T',MAX_PTS*OP_NUM,dofs_per_element,dofs_per_element,&
               1.,fterm,MAX_PTS*OP_NUM,cl,dofs_per_element,0.,mu79,MAX_PTS*OP_NUM)
  ...
  !===============================================
  subroutine eval_ops(itri,fin,outarr)
    use field
    implicit none
    ...
    call dgemv('N',MAX_PTS*OP_NUM,dofs_per_element,1.,nu79,MAX_PTS*OP_NUM,dofs,1,0.,outarr,1)
  ...
  ! evaluates linear unitary operators
  !===============================================
  subroutine eval_ops_old(avector,ngauss,outarr)
    implicit none
  ...
  subroutine define_fields(itri)
    ...
    call precalculate_terms(xi_79,zi_79,eta_79,d%co,d%sn)
    call define_basis(itri)

    ! PSI
    ! ~~~
    call eval_ops(itri, psi_field, psi79)

    ! ETA
    ! ~~~
    call eval_ops(itri, eta_field, eta79)    

    ! define a lot of fields
    do i=1, nfield
       call eval_ops(itri, eta_field, f79(:,:,i))
    end do
