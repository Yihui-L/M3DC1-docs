Program m3dc1_skeleton
  ...
  integer :: nDevices, iDevice

  call MPI_Init(ierr)
  call MPI_Comm_rank(MPI_COMM_WORLD, myrank, ierr)
  call MPI_Comm_size(MPI_COMM_WORLD, maxrank, ierr)

  nDevices=acc_get_num_devices(acc_get_device_type())
...
!$acc routine
  ...
  print *, 'Stopped with output ', out
  stop(out)
end subroutine d_safestop
