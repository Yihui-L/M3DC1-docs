pro eqn_momentum_x, _EXTRA=extra, nterms=nterms, terms=term, names=names, $
               title=title, x=x, z=z
  title = 'Momentum Equation: R'
  nterms = 3
...
;        chi1_z = den0*0.
     ...
     if(ivform eq 1) then begin
        vx1 = -r*u1_z + chi1_r/r^2
     endif else begin
        vx1 = -u1_z/r + chi1_r/r^2
     endelse
...
;     jx1 = -(i1_z + f1_z*rfac^2)/r + rfac*psi1_r/r^2
     ...
     term[0,*,*] = den0*vx1 * gamma[0]

     ; -JxB
     print, 'defining JxB'
     term[1,*,*] = -(jy1*bz0 - jz1*by0 + jy0*bz1 - jz0*by1)
