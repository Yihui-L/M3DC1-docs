...
克隆仓库后，也可以直接在浏览器中打开根目录的 `index.html`：
...
- `index.html`：参数指南网页与 GitHub Pages 入口。
- `docs-data/`：完整参数表、简化表、源码使用统计、官方文档差异清单和输入模板。
- `M3DC1/`：用于核对参数行为的上游 M3D-C1 源码快照，不包含上游 `.git` 历史。
- `tools/extract_m3dc1_params.py`：从源码重新生成参数资料的脚本。
...
```bash
python3 tools/extract_m3dc1_params.py
```
